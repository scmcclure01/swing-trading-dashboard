...
A Complete Systematic Rules-Based Methodology — Version 4.0

Hold Period: 1–12 Weeks  |  Platform: TradingView  |  Universe: All US Equities  |  Account: Equities Only (No Options)

*Incorporating: Prometheus Research + Zero-Cost Public Data Enhancements + Sector Rotation ETF Layer + Core-Satellite Structure*
...
# **Portfolio Structure — Core-Satellite [v4]**
...
Exit any Core ETF when ANY of the following occurs:
1. Sector ETF closes below 20d MA on above-average volume
2. RS line vs SPY declines for 2+ consecutive weeks
3. Regime shifts and no longer favors the sector
...
The macro regime filter is the top-level permission system. Before looking at any stock or sector, you must understand the macroeconomic environment. This determines: (1) whether to trade at all, (2) which sectors to favor, (3) whether momentum or mean-reversion will dominate, and (4) whether any sector is accelerating fast enough to trigger the Velocity Flag [v4].
...
A normal rotation and a white-hot melt-up require different entry tactics. The framework's standard entry criteria (breakout on 40%+ volume, or pullback to 20d MA) work in 7 out of 10 markets. In the 3 out of 10 where a sector goes parabolic, the pullback never comes and the standard rules keep you in cash while the move compounds.

The Velocity Flag detects when a sector is moving fast enough to warrant an alternate entry protocol.
...
- Maximum 3 Accelerating entries simultaneously (across all sectors)
- Accelerating entries STILL require regime alignment — no chasing defensive sectors in a Risk-On regime
- Earnings blackout (10 business days) still applies — no exceptions
- If permission state is YELLOW, Accelerating entries are at quarter size (not half)
- If permission state is RED, no Accelerating entries

## **How Regime Changes Your Trading Style**

| **Regime** | **Trading approach** |
| --- | --- |
| Risk-on | Buy breakouts aggressively. Hold winners longer. Full exposure. Momentum dominates. Deploy Core into leading sectors. |
| Reflation | Momentum works in energy/commodities, may reverse in tech. Tighten tech stops. Rotate to commodity/industrial. Core follows rotation. |
| Deflationary / recession risk | Mean-reversion traps everywhere. Bounces fail. Smaller positions, tighter stops. Do not chase. Core reduces to 1 ETF or cash. |
| Stagflation / tightening liquidity | Almost nothing works on the long side. Cash is a position. 3-5 strongest names only or move to cash. Core exits entirely. |
...
| 4. Velocity Flag check [v4] | Calculate ROC 21 for each sector ETF. Flag any sector > +15%. Note which sectors are Accelerating. |
| 5. Bond confirmation | TLT direction vs SPY. Confirms or challenges regime reading. |
| 6. HYG/IEF spread | Falling ratio = credit spreads widening = liquidity tightening override. |
...
Once a month (after CPI and PCE releases), run a valuation check to assess whether equities are rich or cheap relative to real interest rates and macro fundamentals. This check directly modifies position sizing across the entire portfolio (Core + Tactical) when equities are materially overpriced vs. macro conditions.
...
- Green to Yellow: SPY 1-month turns negative while 6-month remains positive, OR recession composite moves to 2-3/5. Core reduces from 40% to 20% [v4].
- Yellow to Red: SPY 6-month turns negative, OR liquidity override triggered, OR recession composite reaches 4+/5. Core exits to cash [v4].
- Red to Yellow: SPY 1-month turns positive AND recession composite falls below 3/5. Light re-engagement with half-size positions. Core may re-enter at 10-15% in strongest sector only [v4].
- Yellow to Green: SPY 6-month turns positive AND recession composite below 2/5. Resume full engagement. Core scales back to 40% [v4].
...
| Risk-on + Velocity Flag active [v4] | Accelerating Protocol — see Layer 0, Signal 10 |
...
## **Entry Trigger Type 3: Accelerating Protocol [v4]**
...
|  |
| Example (Tactical entry): |
| Account Value: $71,000 |
| Risk Per Trade: 0.75% = $533 |
| Entry Price: $197.00  │  Stop Price: $190.00  │  Risk Per Share: $7.00 |
| Shares to Buy: $533 / $7.00 = 76 shares |
| Position Value: 76 x $197 = $14,972 (21% of account — triggers 10% cap) |
| Capped shares: $7,100 / $197 = 36 shares |

Hard cap: No single position exceeds 10% of total account value regardless of formula output. Core positions have a separate 15% cap per ETF [v4].
...
| Red | No new positions | No new ETF entries | Exit all Core to cash |

## **Accelerating Protocol Sizing [v4]**

When the Velocity Flag is active and an Accelerating entry is taken:
1. Calculate normal position size per the formula above
2. Halve the share count (risk dollars remain approximately constant because stop is tighter)
3. Verify position value is under 10% cap
4. Maximum 3 simultaneous Accelerating positions
...
## **Minimum Deployment Floor [v4]**

| **Permission State** | **Minimum Deployed Capital** | **How to Fill** |
| --- | --- | --- |
| GREEN | 40-60% of account | Core ETFs fill the floor. If no Tactical entries meet criteria, Core carries the allocation alone. |
| YELLOW | 20-35% of account | Core at half allocation (20%). Tactical selective. |
| RED | No floor — cash preservation overrides | Exit Core. Exit most Tactical. Capital preservation is the priority. |

**Purpose:** The deployment floor prevents the framework from keeping 85%+ of the account in cash during strong trending markets. Core ETF positions provide persistent, regime-aligned exposure regardless of whether individual stock entry criteria are met.

**Enforcement:** During the weekly review, check total deployed capital (Core + Tactical). If below the floor for the current permission state, evaluate whether additional Core ETF positions can be established in Phase 2 sectors. If no Phase 2 sectors exist, the floor is not forced — do not enter low-conviction positions to meet a target.
...
## **Partial Profit Taking (Tactical Individual Stocks)**
...
### **Accelerating Protocol Profit Taking [v4]**

| **Profit level** | **Action** |
| --- | --- |
| +8-10% from entry | Sell 1/3 of position. Move stop on remainder to breakeven. |
...
- 12-week max hold reached with insufficient progress (6 weeks for Accelerating entries [v4])

### **Core ETF positions [v4] — exit any of:**
- Close below 20d MA on above-average volume
- RS line vs SPY declines for 2+ consecutive weeks
- Regime shifts and no longer favors the sector
...
- Maximum hold: 12 weeks regardless of whether stop has been hit (6 weeks for Accelerating entries [v4])
...
| --- | --- |
| 0-7% drawdown | Normal operations. Review open positions. Core holds if stops intact. [v4: expanded from 0-5%] |
| 7-10% drawdown | Caution zone. Reduce risk per trade by 50%. No new Tactical positions. Core holds if stops intact but no new Core entries. Tighten all stops. [v4: adjusted from 5-10%] |
| 10-15% drawdown | Emergency protocol. Max 3-5 positions (including Core). Exit all Tactical. Core may hold 1 strongest ETF if stop intact. No new entries. [v4: adjusted from 10-15%] |
| >15% drawdown | Full stop. Exit all positions including Core. 100% cash. Wait for confirmed green state before re-engaging. |

**Rationale for recalibration [v4]:** With 40% of the account deployed in Core positions, a 10% sector correction during normal market activity creates ~$2,800 in Core losses. The prior Tier 1 at -5% ($3,556) would trigger during routine pullbacks, creating excessive whipsawing. The new -7% Tier 1 ($4,978) gives Core positions room to breathe while still protecting against sustained drawdowns.
...
Note: Hedge instruments are equities-only compatible. No options available in BrokerageLink.
...
- Core scales back to 40% only after full green state + 50% drawdown recovery [v4]
...
| 7. Calculate position size: (Account x Risk%) / (Entry - 10d EMA), then HALVE shares |
| 8. Does adding this position stay within portfolio limits? |
| 9. If all yes: enter at half size, stop at 10d EMA, 6-week max hold |
| 10. Record: entry price, stop (10d EMA), T1 (+8-10%), T2 (+15-20%), max hold date (entry + 6 weeks) |

| **Per-Trade Management (Weekly review of open positions)** |
| --- |
| Core ETF positions [v4]: |
| 1. Check ETF 20d MA: still above it? → If below on volume: exit |
| 2. Check RS line: declining 2+ weeks vs SPY? → Exit |
| 3. Check regime: still favorable for this sector? → If not: exit |
...
| 2. Has the +15-20% second target been reached? → Sell another 1/3, trail at 20d MA |
| 3. Has the 10d EMA stop been violated? → Exit (pre-T1), or 20d MA (post-T1) |
| 4. Has the 6-week max hold been reached? → Exit |
| 5. Has the Velocity Flag deactivated? → Manage as standard from this point |
...
| **Velocity Flag status [v4]** | Sectors with ROC 21 > +15%: ____ │ Accelerating sectors: ____ |
...
| **State** | **Max positions / Risk / Approach** | **Core Allocation [v4]** |
| --- | --- | --- |
| Green | Up to 20 / 0.75-1.0% risk / Momentum breakouts | Full — up to 40% in 2-3 ETFs |
| Yellow | 8-12 / 0.25-0.5% risk / Both setups | Half — up to 20% in 1-2 ETFs |
| Red | 3-5 or cash / No new entries (stocks or ETFs) | ZERO — exit all Core |
...
| **State** | **Min Deployed** | **Filler** |
| --- | --- | --- |
| Green | 40-60% | Core ETFs |
...
Shares = (Account x Risk%) / (Entry - Stop). Max single Tactical position = 10% of account. Max single Core ETF = 15% of account [v4].

## **Profit Taking Rules — Standard**
...
## **Profit Taking Rules — Accelerating Protocol [v4]**

| **Profit level** | **Action** |
| --- | --- |
| +5% | Move stop to breakeven |
| +8-10% | Sell 1/3, stop to breakeven |
| +15-20% | Sell another 1/3, trail remainder at 20d MA |
| Remainder | Trail at 20d MA, hold up to 6 weeks |

## **Drawdown Emergency Levels [v4: Recalibrated]**

| **Drawdown from peak** | **Action** |
| --- | --- |
| 0-7% | Normal operations. Core holds. |
| 7-10% | Cut risk/trade 50%, no new Tactical. Core holds, no new Core. |
...
| **Signal** | **Action** |
| --- | --- |
| ETF closes below 20d MA on volume | Exit Core ETF |
| RS vs SPY declining 2+ weeks | Exit Core ETF |
| Regime no longer favors sector | Exit Core ETF |
| Permission state goes RED | Exit ALL Core |
| 2 consecutive weeks outflows (ETF.com) | Exit Core ETF |
...
*Framework v4.0. Based on Prometheus Research methodology (prometheus-research.com) augmented with zero-cost public data signals from FRED, CFTC, Atlanta Fed, FactSet, and ETF.com sector flow data. Core-Satellite structure and Velocity Flag added May 2026 based on program evaluation findings. For systematic swing trading with 1-12 week hold periods.*
